...
class encoder_LSTM(nn.Module):
    def __init__(self, vocabulary, emb_dim, enc_hidden_size, z_hidden_size, hidden_sizes):
        assert isinstance(vocabulary, DataSet.Vocabulary)
        super(encoder_LSTM, self).__init__()
        self.embedding = nn.Embedding(num_embeddings=vocabulary.count,
                                      embedding_dim=emb_dim, padding_idx=vocabulary.pad_token)
        # self.rnn = nn.RNN(hidden_size=hidden_size, input_size=emb_dim, dropout=0.2, batch_first=True)
        self.rnn = nn.LSTM(hidden_size=enc_hidden_size, input_size=emb_dim,
                           bidirectional=False, batch_first=True, num_layers=1)
        self.mlp = MLP([enc_hidden_size + 2] + hidden_sizes + [[z_hidden_size, z_hidden_size]],
                             activation=nn.Softplus,
                             output_activation=[None, Exp],
                             allow_broadcast=True,
                             use_cuda=True)

    def forward(self, inseq, *input):
        sequences, ys = inseq
        embs = self.embedding(sequences)
        X_lengths = []
        for i, lab in (sequences == 1).nonzero():
            X_lengths.append(lab)
        embs = torch.nn.utils.rnn.pack_padded_sequence(embs, X_lengths, batch_first=True)
        output, (h_n, c_n) = self.rnn(embs)
        return self.mlp([h_n[-1], ys])
...
class classifier_LSTM(nn.Module):
    ...
    def forward(self, sequences, *input):
        embs = self.embedding(sequences)
        X_lengths = []
        for i, lab in (sequences == 1).nonzero():
            X_lengths.append(lab)
...
class decoder_LSTM(nn.Module):
    def __init__(self, vocabulary, hidden_size= 35, z_hidden_size=16, output_size = 2):
        assert isinstance(vocabulary, DataSet.Vocabulary)
        super(decoder_LSTM, self).__init__()
        self.rnn = nn.LSTM(hidden_size=hidden_size, input_size=z_hidden_size+2,
                           bidirectional=False, batch_first=False, num_layers=1)
        self.lin = nn.Linear(in_features=hidden_size, out_features=vocabulary.count)
        self.activation = nn.Softmax(-1)


    def forward(self, inseq, input_size, *input):
        zs, ys = inseq
        zy = torch.cat((zs,ys),-1)
        if len(zy.size()) < 3:
            zy = zy.unsqueeze(0)
        out, (h_n, c_n) = self.rnn(zy)
        lin = self.lin(out)
        output = self.activation(lin)
        output = output.unsqueeze(-2)

        for _ in range(input_size-1):
            out, (h_n, c_n) = self.rnn(zy, (h_n, c_n))
            lin = self.lin(out)
